...
% DOCUMENT METADATA
% ============================================================
\title{Certifying $\Gamma$-Robust Discrete Pricing via Full-Breakpoint Multiple-Choice Knapsack Decomposition}
...
\begin{abstract}
We study finite-menu portfolio pricing under a ratio margin requirement, price-admissibility constraints, and integer-budget demand uncertainty.  The pricing model reduces exactly to a multiple-choice knapsack problem (MCKP), while the coupled robust constraint is equivalent to a finite family of fixed-threshold MCKPs indexed by zero and every original option deviation.  For each threshold, upper-hull geometry yields an exact LP bound and a feasible one-item rounding rule whose loss is bounded by a single adjacent hull-value jump; under bounded jumps and linear objective growth, the corresponding relative bound is $O(1/n)$.  Direct evaluation of the original sorted-$\Gamma$ protection term certifies every returned pricing decision.  Numerical experiments on synthetic portfolios and a stylized retail case study illustrate the fixed-threshold certificate and the revenue--protection tradeoff.  The paper's contribution is the pricing reduction, complete full-breakpoint decomposition, and fixed-threshold certification framework; simultaneous bounding of the entire threshold family is outside its scope.
\end{abstract}
...
\item[(ii)] We introduce a $\Gamma$-budget robust extension of this MCKP and prove that the coupled robust constraint admits a \emph{parametric dual decomposition} (\Cref{thm:parametric}) over the complete set consisting of zero and every original admissible option deviation (\Cref{prop:theta_candidates_discrete_clean}). This yields a specialized structure-exploiting framework without tolerance-clustering or discarding thresholds required for exactness.
...
The exact layer in \Cref{sec:exact} separates the role of the hull as an LP
bound from the original integer option set, preserves a valid record for every
full-breakpoint subproblem, and therefore supports either an exact optimum or a
valid anytime global gap.  The guarded sweep changes how fixed-threshold data
are constructed without changing the threshold family or certificate
semantics.

Several directions remain open.  A guarded parametric sweep can update transformed coefficients over the complete threshold set and reuse a hull only when its defining point set is unchanged.  A different subsequent question is whether the largest LP value over the entire threshold family can be bounded simultaneously, rather than by solving every fixed-threshold relaxation; that development is not claimed here.
